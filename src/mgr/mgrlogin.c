...
{
    while (*s) {
        printchar(x, y, *s++); x += font->head.wide;
    }
}
/* edit */
    ...
    while (1) {
        ...
        default:
        {
            if (len < 8) {
                ...
                if (visible) {
                    printchar(x, y, c); x += font->head.wide;
                }
            }
        }
        }
...
int main(int argc, char *argv[])
{
    ...
    if (fontname) {
        ...
        if (*fontname == '/' || *fontname == '.') {
            ...
            strcpy(fontpath, ICONDIR); strcat(fontpath, "/"); strcat(fontpath, fontname);
        }
    ...
    if (background) {
        ...
        if (*background == '/' || *background == '.') {
            ...
            strcpy(backgroundpath, ICONDIR); strcat(backgroundpath, "/"); strcat(backgroundpath, background);
        }
        ...
        *loginstr = '\0'; *passwordstr = '\0';
        ...
        if (ret == 27) {
            ...
            if ((pw = getpwnam(loginstr)) != (struct passwd *)0 && strcmp(crypt(passwordstr, pw->pw_passwd), pw->pw_passwd) == 0) {
                ...
                char *login_env[7] = {
                    env_user, env_logname, env_home, env_shell, env_path, env_mail, (char *)0
                };
                char *login_argv[2] = {
                    "mgr", (char *)0
                };
                ...
                if (ttyname(0)) {
                    chown(ttyname(0), pw->pw_uid, pw->pw_gid); chmod(ttyname(0), 0600);
                }
